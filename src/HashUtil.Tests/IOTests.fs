...
type DisplayTests(output: ITestOutputHelper) =
    ...
    member _.``Print file hash``() =
        ...
        printHashStructure hashStructure false false strWriter true
        ...
        printHashStructure hashStructure true false strWriter true
        Assert.Equal(expectedStr, strWriter.ToString())

    [<Fact>]
    member _.``Print file hash only``() =
        let hashStructure =
            ItemHash.File(path = "/path/to/file.txt", hash = "a1b2c3")
        ...
        // Output without tree should be a single line.
        printHashStructure hashStructure false true strWriter true
        let expectedStr = "a1b2c3\n"
        Assert.Equal(expectedStr, strWriter.ToString())

        // With tree should be same result.
        strWriter.GetStringBuilder().Clear() |> ignore
        printHashStructure hashStructure true true strWriter true
        Assert.Equal(expectedStr, strWriter.ToString())

    [<Fact>]
    member _.``Print dir tree hash only``() =
        let hashStructure =
